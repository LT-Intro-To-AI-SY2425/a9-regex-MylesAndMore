...
# matches anything that starts with a word character (alphanumeric or underscore) then whitespace,
# then one or more digits, a comma and whitespace, and finally one or more digits
...
# matches one or more digits (address number), followed by whitespace, then one or more
# alphanumeric characters (street), followed by whitespace, then one or more word characters
# (city), followed by a comma and whitespace, then two word characters (state), followed
# by whitespace, and finally five digits (zip code)
pat = re.compile("(?P<number>\d+)\s+(?P<street>.+)\s+(?P<city>[\w]+),\s+(?P<state>[\w]{2})\s+(?P<zip>\d{5})", re.IGNORECASE)
address_matches = pat.match(address_string)

# problem 3
# should match all hashtags
tweet_string = "hi everyone! #cs #python #LT #champions"
# matches anything that starts with a # and is followed by one or more alphanumeric characters
